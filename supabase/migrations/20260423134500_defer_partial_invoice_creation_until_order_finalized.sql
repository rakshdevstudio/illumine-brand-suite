-- Defer invoice creation until order assembly is complete.
--
-- Purpose:
-- - Prevent checkout from failing when legacy triggers call invoice creation too early
-- - Keep invoice totals exact once all order_items are present
-- - Preserve invoice rebuild, validation, and ledger posting behavior
...
BEGIN
  ...
  IF abs(v_total - v_order_total) > 0.01 THEN
    RAISE NOTICE 'Invoice creation deferred: invoice_total=%, order_total=%', round(v_total, 2), v_order_total;
    RETURN NULL;
  END IF;
