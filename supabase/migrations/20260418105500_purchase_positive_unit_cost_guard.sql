-- Guard against zero-value purchase ledger lines.
-- Ensures create_purchase_with_ledger never generates debit/credit = 0 entries.
...
BEGIN
  ...
  LOOP
    ...
    IF COALESCE((v_item->>'unit_cost')::numeric, 0) <= 0 THEN
      RAISE EXCEPTION 'Purchase unit_cost must be > 0';
    END IF;
    ...
    IF v_line_base <= 0 THEN
      RAISE EXCEPTION 'Each purchase line must have positive base amount';
    END IF;
  ...
  IF v_subtotal <= 0 OR v_total <= 0 THEN
    RAISE EXCEPTION 'Purchase totals must be > 0';
  END IF;
