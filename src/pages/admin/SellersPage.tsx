...
const SellersPage = () => {
  ...
  const toggleSellerStatus = useMutation({
    mutationFn: async ({ sellerId, currentStatus }: { sellerId: string; currentStatus: string }) => {
    ...
    onError: (err: any) => toast.error(err.message || "Failed to update seller status."),
  });

  const updateCommission = useMutation({
    mutationFn: async ({ sellerId, rate }: { sellerId: string; rate: number }) => {
      const { error } = await (supabase as any).from("sellers").update({
    ...
    onError: (err: any) => toast.error(err.message || "Failed to update commission rate."),
  });
              ...
              <TableRow key={row.id}>
                ...
                <TableCell>
                  <div className="flex items-center gap-2">
                    <Input
                      type="number"
                      defaultValue={row.commission_rate ?? 0}
                      onBlur={(e) => {
                        const newRate = Number(e.target.value);
                        if (newRate !== row.commission_rate) updateCommission.mutate({ sellerId: row.id, rate: newRate });
                      }}
                ...
                <TableCell>
                  <Button
                    ...
                    disabled={toggleSellerStatus.isPending && toggleSellerStatus.variables?.sellerId === row.id}
                    onClick={() => toggleSellerStatus.mutate({ sellerId: row.id, currentStatus: row.status ?? "inactive" })}
                  >
